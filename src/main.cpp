...
uint256 hashGenesisBlock("0xbec049ef51ba60abe4bbf5d9660330666440271a6576376bd104ee3e884d5509");
...
{
    // PeopleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // PeopleCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "For The People";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1438289000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1579796;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xe403711b32cd0fb46a3e05a35e8b845f6340188a93442ef1779f40a28396c567"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xca, 0xa1, 0xe2 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PeopleCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PeopleCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PeopleCoinMiner(CWallet *pwallet)
{
    printf("PeopleCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PeopleCoin-miner");
...
//            MilliSleep(1000);
        ...
        printf("Running PeopleCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PeopleCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PeopleCoinMiner, pwallet));
}
