...
// Copyright (c) 2011-2012 PeopleCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("PeopleCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #PeopleCoinTEST0\r");
            Send(hSocket, "WHO #PeopleCoinTEST0\r");
        } else {
            // randomly join #PeopleCoin00-#PeopleCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #PeopleCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #PeopleCoin%02d\r", channel_number).c_str());
        }
